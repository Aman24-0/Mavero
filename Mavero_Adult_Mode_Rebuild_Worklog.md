...
## Post-release fix phase (2026-09-08) — implementation note

Concise note only; the Phase 1–10 rebuild history above is unchanged and no Adult Mode authorization/classification/registry semantics were modified.

- **Indian Popular TV daily soaps**: root cause — TMDB tags Indian daily soaps with ONLY Drama (18) (Patiala Babes /tv/85879, Vantalakka /tv/235424, Pallakilo Pellikuturu /tv/235330, Meenakshi Ponnunga /tv/276583, Bhoomige Bandha Bhagyantha /tv/275535 all measured Drama-only, 198–957 episodes), so the Phase 8 Soap/News/Talk genre exclusion cannot see them. Fix: isolated curation policy (`src/lib/server/content/popular-tv-policy.ts`) — a candidate whose cached TV detail reports >100 released episodes is a serial and is dropped, in the Popular TV rail (series half) ONLY; deterministic survivor walk; policy dimension appended to the `tmdb:popular-v2` cache key; detail failure keeps the candidate (curation fail-open); the unconditional adult exclusion and every Phase 8 query constraint are untouched.
- **Adult default playback source**: `streaming_default_sources` CHECK extended to `movie|series|anime|adult` (ALTER-only migration `20260914000000_adult_default_source.sql`; data/FK/RLS/triggers preserved). Same source-based contract and eligibility rules; 4th Adult card on `/admin/defaults`; the watch page uses `defaults.adult` for Adult-tagged titles — which render ONLY behind the existing Phase 6 guard, so the default never affects authorization and Adult Mode OFF keeps adult content inaccessible.
- **Adult Discover provider filter**: closed union `provider = all | <verified registry key>` on `/api/content/adult-discover` (400 otherwise; raw ids never accepted); adapter maps the key via the verified registry (`Adult AND provider`, never OR; movies fail closed). `/api/discover/adult-providers` now serves the VERIFIED network registry with captured TMDB logo paths (same image CDN convention). The language dropdown was removed from the Adult surface (server contract still validates the dimension, default `all`). The Movies filter's zero-result state was re-audited and is the documented fail-closed transitional under-fill (no adult watch-providers resolve in TMDB's India catalog) — preserved intentionally.
- **Header row**: the Adult section heading row no longer wraps at any width (title truncates; the provider filter collapses to an icon-only control under 480px) — supersedes the Phase 10 `flex-wrap` mitigation.
- **Admin area**: `/admin/*` renders bare in the root layout (consumer AppShell/bottom nav never mounted there; AdminShell keeps its own navigation).
- Tests: new suites `popular_tv_soap_policy_test.ts`, `adult_default_source_test.ts`, `adult_discover_provider_filter_test.ts`, `admin_nav_test.ts`; deliberate updates to assertions that encoded the superseded contracts in `adult_mode_test.ts`, `adult_phase8_ui_test.ts`, `adult_phase9_final_test.ts`, `phase7_admin_defaults_test.ts`. Baseline trio PASS (test exit 0; check 0 errors/38 warnings = pre-existing baseline; build PASS).
