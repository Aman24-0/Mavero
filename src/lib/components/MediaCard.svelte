...
<div class:compact class:featured class="card-wrap">
...
<style>
  ...
  .card-wrap.featured .poster { border-color: rgba(248,250,252,.11); box-shadow: 0 10px 28px rgba(0,0,0,.24); }
  .card-wrap.featured .card-copy { padding-top: 11px; }
  .card-wrap.featured .card-title { font-family: 'Space Grotesk', sans-serif; font-weight: 600; }
  .card-wrap.featured .card-meta { color: var(--muted); }
  .card-wrap.featured .poster-type { color: rgba(248,250,252,.82); }
  .card-wrap.featured .card-tag { border-color: rgba(248,250,252,.12); background: rgba(9,10,12,.52); }
  .card { display: block; }
  .poster { isolation: isolate; }
  .poster-type { position: absolute; right: 10px; bottom: 10px; z-index: 1; color: rgba(248,250,252,.72); font-family: 'DM Mono', monospace; font-size: .48rem; letter-spacing: .1em; text-transform: uppercase; }
  .poster-action { position: absolute; right: 10px; bottom: 30px; z-index: 2; display: grid; place-items: center; width: 28px; height: 28px; border: 1px solid rgba(248,250,252,.24); border-radius: 50%; color: var(--ink); background: rgba(10,12,16,.68); opacity: 0; transform: translateY(5px); transition: opacity 180ms var(--ease-out), transform 180ms var(--ease-out), background 180ms var(--ease-out); }
  .card:hover .poster-action, .card:focus-visible .poster-action { opacity: 1; transform: translateY(0); background: rgba(167,139,250,.86); color: #101010; }
  .card-detail-link { position: absolute; right: 8px; bottom: 57px; z-index: 2; display: inline-flex; align-items: center; gap: 4px; border: 1px solid rgba(248,250,252,.2); border-radius: 8px; padding: 5px 8px; color: var(--ink); background: rgba(10,12,16,.83); font-family: 'DM Mono', monospace; font-size: .5rem; text-decoration: none; opacity: 0; transform: translateY(4px); transition: opacity 160ms var(--ease-out), transform 160ms var(--ease-out), border-color 160ms var(--ease-out); }
  .card-wrap:hover .card-detail-link, .card-detail-link:focus-visible { opacity: 1; transform: translateY(0); border-color: rgba(167,139,250,.58); }
