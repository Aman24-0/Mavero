<script lang="ts">
  import { ArrowRight, Mail, LockKeyhole, UserRound, ShieldCheck, AlertCircle, CheckCircle2 } from 'lucide-svelte';
...
>
  ...
  {#if !form?.success}
    <form method="POST" action="?/signUp" class="auth-form">
      ...
      <label class="field">
        ...
        <div class="input-wrap">
          ...
          <input name="password" type="password" placeholder="At least 6 characters" autocomplete="new-password" minlength="6" required />
        </div>
      </label>
    ...
    <details class="resend-details">
      <summary>Already created an account?</summary>
      <form class="auth-form inline-form" method="POST" action="?/resend">
        <label class="field">
...
<style>
  ...
  .resend-details summary::-webkit-details-marker { display: none; }
  .resend-details summary::before {
    content: '›'; display: inline-block; margin-right: 6px;
    color: #77777f; transition: transform 180ms ease;
  }
  .resend-details[open] summary::before { transform: rotate(90deg); }
  .resend-details summary:hover { color: #f5f5f5; }
  ...
  @media (prefers-reduced-motion: reduce) {
    .primary-cta, .secondary-cta, .input-wrap, .resend-details summary::before { transition: none; }
  }
</style>
