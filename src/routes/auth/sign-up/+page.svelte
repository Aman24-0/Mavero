...
<div class="auth-wrap container-wide"><a class="back-link" href="/profile"><ArrowLeft size={15} /> Back to Profile</a><div class="auth-card"><div class="eyebrow">MAVERO / Create account</div><h1>Your next story<br /><em>travels with you.</em></h1><p>Create an account to keep your progress, watchlist, and history synced across devices.</p><form onsubmit={(event) => event.preventDefault()}><label>Name<div class="auth-input"><UserRound size={15} /><input bind:value={name} placeholder="Your name" required /></div></label><label>Email<div class="auth-input"><Mail size={15} /><input bind:value={email} type="email" placeholder="you@example.com" required /></div></label><label>Password<div class="auth-input"><LockKeyhole size={15} /><input bind:value={password} type="password" placeholder="At least 8 characters" minlength="8" required /></div></label><button class="btn btn-primary" type="submit">Create account</button></form><div class="auth-note"><ShieldCheck size={15} /> Your current guest progress will be offered for sync after sign-up.</div></div></div>
