...
<div class="container-wide profile-page">
  <section class="profile-header">
    ...
    <div class="identity-copy"><div class="eyebrow">MAVERO / Your space</div><h1>{accountName()}<span>.</span></h1><p>{isAuthenticated ? 'Cloud library' : 'Local library'}</p><div class="identity-meta"><span class:online={syncStatus === 'synced'}></span>{isAuthenticated ? syncStatusLabel(syncStatus) : 'Guest profile · Local library'}</div></div>
  ...
  <section class="profile-grid">
    <section class="profile-card profile-card-main"><div class="profile-card-row"><div><div class="eyebrow">{isAuthenticated ? 'Cloud library' : 'Guest mode'}</div><strong>{loaded ? 'Ready' : 'Loading'}</strong></div><span class="sync-badge"><ShieldCheck size={14} /> {syncStatusLabel(syncStatus)}</span></div><div class="sync-row"><span>{storageMessage}</span></div></section>
...
<style>
  ...
  .profile-card { min-height: 122px; padding: 18px; border: 1px solid var(--line); border-radius: var(--radius-lg); background: var(--surface); }
  .profile-card-main { background: radial-gradient(circle at 88% 18%, rgba(85,214,194,.12), transparent 20rem), var(--surface); }
  .profile-card-row { display: flex; align-items: center; justify-content: space-between; gap: 14px; } .profile-card-row strong { display: block; margin-top: 8px; color: var(--ink); font-family: 'Space Grotesk', sans-serif; font-size: 1.65rem; font-weight: 600; } .sync-badge { display: inline-flex; align-items: center; gap: 6px; color: var(--secondary); font-family: 'DM Mono', monospace; font-size: .56rem; text-transform: uppercase; }
  ...
  .cinelog-banner p { margin: 0; color: var(--muted); font-size: .72rem; }
