...
export const DELETE: RequestHandler = async ({ locals, url }) => {
  ...
  const { error } = await locals.supabase.from('favorites').delete().eq('user_id', user.id).eq('favorite_key', favoriteKey(contentType, contentId));
  if (error) return json({ message: 'Cloud favorite removal failed.' }, { status: 503 });
  return json({ ok: true });
};
