...
<style>
  ...
  .filter-trigger:hover, .filter-trigger:focus-visible { color: var(--ink); border-color: rgba(255, 62, 94,.6); outline: 0; box-shadow: 0 0 0 3px rgba(255, 62, 94,.09); }
  .filter-trigger:active { transform: scale(.98); }
  .filter-trigger.active { border-color: rgba(255, 62, 94,.68); background: rgba(255, 62, 94,.1); box-shadow: inset 0 0 0 1px rgba(255, 62, 94,.12); }
  .select-label { color: var(--muted-deep); font-family: 'Inter', ui-sans-serif, system-ui, sans-serif; font-size: .52rem; letter-spacing: .08em; text-transform: uppercase; }
  ...
  .select-icon { display: grid; flex: 0 0 21px; place-items: center; width: 21px; height: 21px; overflow: hidden; border-radius: 6px; color: var(--ink); background: var(--accent-soft); font-family: 'Inter', ui-sans-serif, system-ui, sans-serif; font-size: .48rem; font-weight: 800; }
  .select-icon img { grid-area: 1 / 1; }
  .select-icon img { width: 15px; height: 15px; object-fit: contain; border-radius: 3px; }
  .search-loading, .search-error { display: flex; align-items: center; justify-content: center; gap: 8px; margin-top: 22px; color: var(--muted); font-family: 'Inter', ui-sans-serif, system-ui, sans-serif; font-size: .62rem; }
  .search-loading :global(svg) { animation: spin 1s linear infinite; }
  .search-error { color: #ffb020; }
  .results-section { margin-top: 32px; }
  .result-summary { display: flex; align-items: baseline; gap: 10px; margin-bottom: 13px; color: var(--muted-deep); font-family: 'Inter', ui-sans-serif, system-ui, sans-serif; font-size: .58rem; text-transform: uppercase; }
  .result-summary strong { color: var(--ink); font-family: 'Inter', ui-sans-serif, system-ui, sans-serif; font-size: .88rem; letter-spacing: -.02em; text-transform: none; }
  ...
  .empty-mark { color: var(--accent); font-family: 'Inter', ui-sans-serif, system-ui, sans-serif; font-size: 1.4rem; }
