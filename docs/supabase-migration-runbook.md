...
| 6 | `20261004000000_device_rpc_ambiguous_column_fix.sql` | **Complete device-RPC correctness fix (REQUIRED)**: (a) the 20261003/20260930 PL/pgSQL bodies referenced `id` / `exchange_code` / `exchange_attempts` / `user_id` / `supabase_session_id` / `revoked_at` UNQUALIFIED — these collide with the functions' `RETURNS TABLE` OUT-parameter names, so **every** RPC call raised `42702: column reference "id" is ambiguous` (the production `claim-rpc-failed` incident); (b) the masked `make_interval(ms => …)` → 42883 trap in the claim lease UPDATE (now `p_lease_ms * interval '1 millisecond'`); (c) the session-TimeZone-dependent `p_now timestamptz default timezone('utc', now())` — `timezone('utc', now())` returns `timestamp without time zone`, and its cast back to `timestamptz` re-interprets the UTC wall clock in the CALLING session's TimeZone (a +05:30 session made a 30 s lease land 5 h 30 m in the past — the `lease_delta −17 970 s` regression; a −04:00 session made every pairing look already expired). The default is now `now()` (the true instant, session-TimeZone-independent). Replaces all five device RPCs (`claim` / `complete` / `release` / `fail` / `register_device_session`) via `CREATE OR REPLACE` with alias-qualified columns (`dpr.` / `ds.`). Function-only: identical signatures (names/types/order) and every default EXCEPT `p_now` verbatim; `SECURITY DEFINER`, `search_path`, state machine, lease semantics, attempt caps, one-time credentials, race-safety and grants all preserved. The app never sends `p_now` (all call sites omit it), so the wire payload is unchanged. |
