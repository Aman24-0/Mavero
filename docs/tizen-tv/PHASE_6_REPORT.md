...
The detail screen presents the normalized poster/backdrop, type, genres, year, runtime or episode count, rating, synopsis, My List action, recommendations, and—for series—season and episode navigation. Episode selection provides a clear selection/status feedback path only. The player, AVPlay, media controls, resolver, and streaming behavior remain outside Phase 6.
...
Samsung Phase 6 hardware QA is intentionally not claimed by this report until the owner executes this checklist on Samsung `UA43AUE60AKLXL`, Tizen `6.0`, TizenBrew `2.0.5`.
