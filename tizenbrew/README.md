...
The reliable identifier for testing the final pushed host-aware exit revision is `Aman24-0/Mavero@aa649f8243f9ae6e98d92aa6e61f0119990cd3f4`. In the current TizenBrew implementation this is passed as the opaque GitHub module path `gh/Aman24-0/Mavero@aa649f8243f9ae6e98d92aa6e61f0119990cd3f4`; jsDelivr resolves the immutable commit ref and exposes the fixed root metadata plus the host-aware exit change. The branch alias `Aman24-0/Mavero@feature/tizen-tv` is also syntactically accepted by jsDelivr, but its package response may remain stale in CDN cache after a push. The UI has no separate branch selector. `Aman24-0/Mavero` without a ref resolves the default branch and is not the correct identifier for testing this feature branch.
