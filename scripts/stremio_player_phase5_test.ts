...
ok(shellSource.includes('pendingSeek = currentTime;') && shellSource.includes('videoElement.currentTime = pendingSeek;'), 'Y: the existing pendingSeek mechanism captures position on switch and restores it on loadedmetadata (HLS restore included)');
