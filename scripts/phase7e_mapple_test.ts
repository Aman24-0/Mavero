...
const providerId = '00000000-0000-4000-8000-0000000007eb';
const sourceId = '00000000-0000-4000-8000-0000000007ec';
const mappleOrigin = 'https://mapple.uk';
...
const capabilities = {
  ...
  allowed_embed_origins: [mappleOrigin]
};
...
const source = {
  ...
  integration_type: 'template' as const,
  capabilities,
  movie_template: `${mappleOrigin}/watch/movie/{tmdb_id}`,
  series_template: `${mappleOrigin}/watch/tv/{tmdb_id}-{season}-{episode}`,
  anime_template: null,
...
assert.equal(episode.url, `${mappleOrigin}/watch/tv/1399-1-1`);
...
await assert.rejects(
  () => resolveSourceFromConfig(
    { sourceId, contentId: '550', mediaType: 'movie' },
    { ...config, source: { ...source, movie_template: 'https://evil.example.test/watch/movie/{tmdb_id}' } },
    content('movie', '550'),
    { adapters: genericAdapters }
  ),
...
console.log('Phase 7E Mapple generic-template and resolver tests passed.');
