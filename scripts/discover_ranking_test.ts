...
assert.match(tmdb, /Boolean\(raw\.poster_path \|\| raw\.backdrop_path\)/);
assert.match(tmdb, /backdrop: image\(raw\.backdrop_path \?\? raw\.poster_path, 'w1280'\)/);
assert.match(tmdb, /backdropSmall: image\(raw\.backdrop_path \?\? raw\.poster_path, 'w780'\)/);
