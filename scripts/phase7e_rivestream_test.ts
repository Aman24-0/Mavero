...
const providerId = '00000000-0000-4000-8000-0000000007e5';
const sourceId = '00000000-0000-4000-8000-0000000007e6';
const riveStreamOrigin = 'https://www.rivestream.app';
...
const capabilities = {
  ...
  allowed_embed_origins: [riveStreamOrigin]
};
...
const source = {
  ...
  name: 'RiveStream Embed',
  status: 'experimental',
  enabled: true,
  visibility: 'public' as const,
  integration_type: 'template' as const,
  capabilities,
  movie_template: `${riveStreamOrigin}/embed?type=movie&id={tmdb_id}`,
  series_template: `${riveStreamOrigin}/embed?type=tv&id={tmdb_id}&season={season}&episode={episode}`,
  anime_template: null,
...
assert.equal(movie.url, `${riveStreamOrigin}/embed?type=movie&id=533535`);
assert.equal(movie.providerId, providerId);
assert.equal(movie.sourceId, sourceId);
assert.equal(movie.sandboxPolicy, 'required');
assert.equal(movie.metadata?.providerName, 'RiveStream');
...
assert.equal(episode.url, `${riveStreamOrigin}/embed?type=tv&id=1399&season=1&episode=1`);
...
await assert.rejects(
  () => resolveSourceFromConfig(
    { sourceId, contentId: '533535', mediaType: 'movie' },
    { ...config, source: { ...source, movie_template: 'https://evil.example.test/embed?type=movie&id={tmdb_id}' } },
    content('movie', '533535'),
    { adapters: genericAdapters }
  ),
