...
const providerId = '00000000-0000-4000-8000-0000000007ed';
const sourceId = '00000000-0000-4000-8000-0000000007ee';
const cinesrcOrigin = 'https://cinesrc.st';
...
const capabilities = {
  ...
  allowed_embed_origins: [cinesrcOrigin]
};
...
const source = {
  ...
  integration_type: 'template' as const,
  capabilities,
  movie_template: `${cinesrcOrigin}/embed/movie/{tmdb_id}`,
  series_template: `${cinesrcOrigin}/embed/tv/{tmdb_id}?s={season}&e={episode}`,
  anime_template: null,
...
function content(type: 'movie' | 'series' | 'anime', tmdb?: string): NormalizedMediaItem {
  return {
    id: type === 'movie' ? '550' : type === 'series' ? '95350' : 'anime-fixture',
    title: 'Fixture title',
    year: 2024,
    type,
    runtime: '120 min',
...
assert.equal(movie.url, `${cinesrcOrigin}/embed/movie/550`);
assert.equal(movie.providerId, providerId);
assert.equal(movie.sourceId, sourceId);
assert.equal(movie.sandboxPolicy, 'required');
assert.equal(movie.metadata?.providerName, 'CineSrc');

const episode = await resolveSourceFromConfig(
  { sourceId, contentId: '95350', mediaType: 'series', season: 1, episode: 1 },
  config,
  content('series', '95350'),
  { adapters: genericAdapters }
);
...
await assert.rejects(
  () => resolveSourceFromConfig(
    { sourceId, contentId: '95350', mediaType: 'series', season: 1 },
    config,
    content('series', '95350'),
    { adapters: genericAdapters }
  ),
...
console.log('Phase 7E CineSrc generic-template and resolver tests passed.');
